...
import { InstructionAudio } from './instruction-audio.js'

/**
 * @typedef {import('../language/glossary.js').GlossPrompt} GlossPrompt
 * @typedef {import('../language/glossary.js').LanguageOption} LanguageOption
 * @typedef {import('../types.js').LanguageCode} LanguageCode
 */

/** @typedef {{ x: number, y: number, z: number }} Transform */

/**
 * @typedef {Object} SceneUIOptions
 * @property {any} sceneEl
 * @property {Transform} position
 * @property {Transform} rotation
 * @property {Transform} scale
 * @property {LanguageOption[]} languages
 * @property {LanguageCode} selectedLanguage
 * @property {(language: LanguageCode) => void} onLanguageChange
 * @property {() => void} onStart
 * @property {() => void} onExit
 * @property {() => void} onAudioReplay
 */

/** @typedef {'success' | 'error'} FeedbackType */
/** @typedef {'pre-game' | 'running'} SceneUIState */
...
/**
 * @typedef {Object} SelectableRow
 * @property {any} root
 * @property {any} background
 * @property {any} label
 * @property {any | null} marker
 * @property {LanguageCode} value
...
export class SceneUI {
  /** @type {Map<LanguageCode, SelectableRow>} */
  languageRows = new Map()
  ...
  constructor(opts) {
    ...
    this.startGroup = document.createElement('a-entity')
    ...
    this.gameGroup = document.createElement('a-entity')
  ...
  /**
   * @param {boolean} countAsReplay
   */
  async replayInstructionAudio(countAsReplay) {
    const played = await this.audio.replay()
    if (played && countAsReplay) this.onAudioReplay()
  }

  /**
   * @param {{ value: LanguageCode, label: string, x: number, y: number, width: number, height: number, selected: boolean, inverse: boolean, marker: boolean, onSelect: () => void }} opts
   * @returns {SelectableRow}
   */
   ...
   */
  createPlane(x, y, width, height, color, z, opacity) {
   ...
   * @param {number} z
   */
  createCircle(x, y, radius, color, z) {
  ...
  /**
   * @param {{ x: number, y: number, z: number, value: string, width: number, wrapCount: number, color: string, size: number, align: 'left' | 'center' }} opts
   */
  createText(opts) {
  ...
  /**
   * @param {{ value: string, width: number, wrapCount: number, color: string, size: number, align: 'left' | 'center' }} opts
   * @returns {Record<string, string | number>}
   */
  getTextAttribute(opts) {
    return {
      value: opts.value,
      width: opts.width,
...
class CanvasTextPanel {
  ...
  /**
   * @param {{ x: number, y: number, z: number, width: number, height: number, color: string }} opts
   */
  constructor(opts) {
  ...
  /**
   * @param {string} text
   * @param {CanvasDirection} direction
   * @returns {{ values: string[], fontSize: number }}
   */
  layoutText(text, direction) {
