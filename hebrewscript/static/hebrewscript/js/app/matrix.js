...
// Vanilla-DOM port of PracticeStatsMatrix.vue - single confusion namespace,
// so the `kind` prop from the source component is dropped (there is only
// ever one matrix). Deliberately keeps the source's quirk of recomputing
// "hardest pairs" from the visible cells rather than using
// summary.topPairs, matching the original component's behavior exactly.
...
/** @param {number} value */
const getScientificColor = (value) => {
...
/** @param {number[]} rgb */
const getRelativeLuminance = ([red, green, blue]) => (0.2126 * red + 0.7152 * green + 0.0722 * blue) / 255;

/** @param {number | null} value */
const formatPercent = (value) => (value === null ? "—" : `${Math.round(value * 100)}%`);

/** @param {number} value */
const formatEffectiveAttempts = (value) => (value >= 10 ? Math.round(value).toString() : value.toFixed(1));

/**
 * @param {MatrixCellStats} cell
 * @param {(key: string) => string} formatKeyLabel
 */
const getTooltip = (cell, formatKeyLabel) => {
 ...
 * @param {{title: string, summary: MatrixSummary, formatKey?: (key: string) => string, onSelectPair: (pairTarget: PracticePairTarget) => void}} options
 */
export const renderMatrix = (container, { title, summary, formatKey, onSelectPair }) => {
  const formatKeyLabel = (/** @type {string} */ key) => formatKey?.(key) ?? key;
