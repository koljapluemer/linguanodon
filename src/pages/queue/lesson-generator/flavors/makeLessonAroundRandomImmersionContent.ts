...
export async function makeLessonAroundRandomImmersionContent(
  ...
  try {
    console.log('Starting makeLessonAroundRandomImmersionContent');
    ...
    console.log(`Generated ${tasks.length} tasks based on immersion content: ${selectedContent.title}`);
    return tasks;
    
  } catch (error) {
    console.warn('Error in makeLessonAroundRandomImmersionContent:', error);
    return [];
  }
}