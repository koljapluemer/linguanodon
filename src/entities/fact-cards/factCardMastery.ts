...
/**
 * Calculate mastery level for a fact card based on ts-fsrs retrievability
 * 
 ...
 * 
 * @param factCard - The fact card to calculate mastery for
 * @returns Mastery percentage (0-100)
 */
export function calculateFactCardMastery(factCard: FactCardData): number {
  const scheduler = fsrs();
  const now = new Date();
  
  // Component 1: Retrievability in 1 day
  const oneDayFuture = new Date(now.getTime() + 24 * 60 * 60 * 1000);
  const oneDayRetrievability = scheduler.get_retrievability(factCard.progress, oneDayFuture, false) as number;
  
  // Component 2: Retrievability in 1 week  
  const oneWeekFuture = new Date(now.getTime() + 7 * 24 * 60 * 60 * 1000);
  const oneWeekRetrievability = scheduler.get_retrievability(factCard.progress, oneWeekFuture, false) as number;
  
  // Component 3: Retrievability in 1 year
  const oneYearFuture = new Date(now.getTime() + 365 * 24 * 60 * 60 * 1000);
  const oneYearRetrievability = scheduler.get_retrievability(factCard.progress, oneYearFuture, false) as number;
