<template>
  <div class="space-y-6">
    <div v-if="goal">
      <DoTaskWidget
        :title="goal.coreTasks.find(t => t.taskType === 'add-vocab-to-goal')?.title || 'Add Vocabulary'"
        :prompt="goal.coreTasks.find(t => t.taskType === 'add-vocab-to-goal')?.prompt || 'Add vocabulary that you need to learn for this goal'"
        @completed="handleTaskCompleted"
        @skipped="handleTaskCompleted"
      />
      
      <div class="divider">Work on Vocabulary</div>
...
async function handleTaskCompleted() {
  ...
  const updatedGoal = await goalRepo.update(goal.value.id, {
    coreTasks: goal.value.coreTasks.map(task => 
      task.taskType === 'add-vocab-to-goal' 
