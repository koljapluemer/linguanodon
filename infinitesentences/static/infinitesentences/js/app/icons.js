// @ts-check
// Port of egyptiansentences'/tprboard's app/icons.js helper - builds SVG DOM
// nodes directly from the lucide CDN UMD build's icon-node arrays
// (window.lucide.<Name>), since there's no build step to use lucide's
 ...
 * @returns {string}
 */
export function iconMarkup(name, sizeClass = "w-5 h-5") {
  const icon = window.lucide[name] ?? window.lucide.HelpCircle;
  return lucideIconMarkup(icon, { class: sizeClass });
}
