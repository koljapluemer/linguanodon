...
import { TONE_KEYS } from "./model.js";

// Pure port of listen-to-viet's src/entities/practice-progress/stats.ts,
// with the "letter" confusion kind removed entirely (dropped field on
// events/candidates, single tone namespace, so pair keys no longer need a
// "kind:" prefix). Also drops buildPairCounts/getBidirectionalPairHistory/
// calculatePosteriorAccuracy, confirmed dead code in the source app (never
// consumed outside stats.ts itself).
...
const getEventPairTarget = (event) =>
  event.correctTone && event.distractorTone ? toBidirectionalPracticePairTarget(event.correctTone, event.distractorTone) : null;
...
const getDirectionalEventPairTarget = (event) =>
  event.correctTone && event.distractorTone ? { correctKey: event.correctTone, distractorKey: event.distractorTone } : null;

/**
 * @param {DistractorCandidate} candidate
 * @returns {PracticePairTarget}
 */
export const getCandidatePairTarget = (candidate) => toBidirectionalPracticePairTarget(candidate.correctTone, candidate.distractorTone);
...
export const getDirectionalCandidatePairTarget = (candidate) => ({
  correctKey: candidate.correctTone,
  distractorKey: candidate.distractorTone,
});
...
export const getPracticeStatsSnapshot = (events) => {
  ...
  return {
    ...
    tone: toMatrixSummary(TONE_KEYS, buildDirectionalPairCounts(trackedEvents)),
  };
};
